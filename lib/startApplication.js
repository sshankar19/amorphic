...
function startApplication(appName, appDirectory, appList, configStore, sessionStore) {
    ...
    return setupDb(dbConfig, schema)
        .then(db => setUpInjectObjectTemplate(dbConfig, config, schema, db))
        .then(boundFunction =>  buildAppConfigAndLoadTemplates(appName, config, controllerJsDir, commonJsDir, sessionStore, boundFunction))
        .spread((baseTemplate, templates) => finishDaemonIfNeeded(config, prop, prefix, appName, baseTemplate, templates));
}
 ...
 *
 * @returns Promise<false | knex handler | mongo handler> db handler or false
 */
function setupDb(dbConfig, schema) {
 ...
 *   simply Persistable.
 *
 * @param {String} dbConfig - The database config
 * @param {Object} config - The app specific config from the config store.
 * @param {String} schema - The app schema.
 *
 * @returns {Function} A bound function to be used when loading templates.
 */
function setUpInjectObjectTemplate(dbConfig, config, schema, db) {
    let amorphicOptions = AmorphicContext.amorphicOptions || {};
    let connectToDbIfNeedBe = Bluebird.resolve(false); // Default to no need.

    return returnBoundInjectTemplate(amorphicOptions, config, dbConfig, schema, db);
}
...
function returnBoundInjectTemplate(amorphicOptions, config, dbConfig, schema, db) {
    ...
    return (objectTemplate) => injectObjectTemplate(amorphicOptions, config, dbConfig, db, schema, objectTemplate);
}
